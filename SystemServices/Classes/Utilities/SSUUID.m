...
+ (NSString *)uniqueID {
    ...
    NSString *sw = [[[NSString stringWithFormat:@"%ld", (long)[SSHardwareInfo screenWidth]] substringToIndex:1] capitalizedString];
    ...
    if (sw == nil || sw.length <= 0 || sw.length > 1 || [sw isEqualToString:@"-"]) {
        // Invalid String
        sw = @"1";
    }
    ...
    UniqueIdentifier = [NSString stringWithFormat:@"%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@", Salt, DeviceModel, SystemName, DeviceType, sw, ScreenHeight, MultitaskingEnabled, ProximitySensorEnabled, NumberProcessors, ProcessorSpeed, CellMACAddress, WiFiMACAddress, WiFiMACAddress2, WiFiMACAddress3, TotalDiskSpace, Salty];
...
+ (NSString *)deviceSignature {
    ...
    // Get the Screen Width
    NSString *sw = [NSString stringWithFormat:@"%ld", (long)[SSHardwareInfo screenWidth]];
    ...
    if (sw == nil || sw.length <= 0 || sw.length > 4) {
        // Invalid String
        sw = @"0000";
    }
    ...
    DeviceSignature = [NSString stringWithFormat:@"%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@%@", SystemVersion, ScreenHeight, sw, PluggedIn, Jailbroken, HeadphonesAttached, BatteryLevel, FullyCharged, ConnectedtoWiFi, DeviceOrientation, Country, TimeZone, NumberProcessors, ProcessorSpeed, TotalDiskSpace, TotalMemory, Salt];
